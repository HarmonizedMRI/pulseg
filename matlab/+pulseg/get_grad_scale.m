...
function test_trap_positive(testCase)
    g.type = 'trap';
    g.amplitude = 20e3;
    testCase.verifyEqual(get_grad_scale(g), 20e3);
end
...
function test_trap_zero(testCase)
    g.type = 'trap';
    g.amplitude = 0;
    testCase.verifyEqual(get_grad_scale(g), 0);
end
...
function test_grad_positive_peak(testCase)
    ...
    testCase.verifyEqual(get_grad_scale(g), 10e3);
end
...
function test_grad_all_positive(testCase)
    ...
    testCase.verifyEqual(get_grad_scale(g), 5e3);
end
...
function test_grad_single_element(testCase)
    g.type = 'grad';
    g.waveform = [7e3];
    testCase.verifyEqual(get_grad_scale(g), 7e3);
end

%% Error Handling Tests

function test_not_a_struct(testCase)
    testCase.verifyError(@() get_grad_scale(42), 'get_grad_scale:invalidInput');
end

function test_missing_type_field(testCase)
    g.amplitude = 10e3;
    testCase.verifyError(@() get_grad_scale(g), 'get_grad_scale:missingField');
end

function test_invalid_type(testCase)
    g.type = 'unknown';
    testCase.verifyError(@() get_grad_scale(g), 'get_grad_scale:unknownType');
end
